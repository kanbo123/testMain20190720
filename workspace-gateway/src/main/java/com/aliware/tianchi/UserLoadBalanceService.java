...
public class UserLoadBalanceService {
    ...
    public static void putLoadInfo(String notiftStr) {
        ...
        System.out.println(String.format("时间:%s,环境:%s,活跃线程数:%s,可用线程数:%s,请求数:%s,平均耗时:%s,平均请求数:%s,权重:%s", 
            nowStr,quota,activeCount,availCount,reqCount,avgTime,avgCountPerThread,serverLoadInfo.getWeight()));
    }
